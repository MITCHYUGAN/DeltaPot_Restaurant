...
export default function CartDrawer({ isOpen, onClose, cartItems, onUpdateQuantity, onRemoveItem, onClearCart }: CartDrawerProps) {
  ...
  // Live calculations
  const totalPriced = cartItems.reduce((sum, item) => sum + item.menuItem.price * item.quantity, 0);
  ...
  const handlePlaceOrder = () => {
    ...
    cartItems.forEach((item) => {
      const priceVal = item.menuItem.price === 0 ? "Price on request" : `₦${(item.menuItem.price * item.quantity).toLocaleString()}`;
      orderListText += `- ${item.menuItem.name} x${item.quantity} — ${priceVal}\n`;
    });

    const totalText = hasPriceOnRequest ? `₦${totalPriced.toLocaleString()} + Price On Request Items` : `₦${totalPriced.toLocaleString()}`;
...
My delivery address:
${address.trim() ? address.trim() : "_[Insert delivery address here]_"}${notes.trim() ? `\n\nSpecial Instructions:\n${notes.trim()}` : ""}`;
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 bg-brand-charcoal/70 backdrop-blur-sm transition-opacity" onClick={onClose} />
        ...
        >
          ...
          <div className="p-5 border-b border-brand-gold/15 flex items-center justify-between bg-brand-dark">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="font-heading text-xl font-bold text-white uppercase tracking-wide">Your Tray ({totalCount})</h2>
            </div>
            <button onClick={onClose} className="p-1.5 text-brand-cream/60 hover:text-brand-red rounded-full hover:bg-white/5 cursor-pointer transition-colors duration-200" aria-label="Close Tray">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            {cartItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-12">
                ...
                <div>
                  <h3 className="font-heading text-lg font-bold text-brand-cream">Your tray is empty</h3>
                  <p className="text-xs text-brand-cream/50 max-w-xs mt-1 font-sans">Browse DeltaPot's legendary Southern menu to fill your tray with smoking hot traditional dishes.</p>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <button onClick={onClearCart} className="text-brand-red hover:underline font-extrabold uppercase tracking-wide flex items-center gap-1 cursor-pointer">
                ...
                <div className="space-y-3.5">
                  {cartItems.map((item) => {
                    const priceCombined = item.menuItem.price === 0 ? "Price on request" : `₦${(item.menuItem.price * item.quantity).toLocaleString()}`;

                    return (
                      <div key={item.menuItem.id} className="p-3.5 bg-brand-dark/50 border border-brand-gold/5 rounded-2xl flex gap-3.5 items-start relative group shadow-sm">
                        {item.menuItem.image && <img src={item.menuItem.image} alt={item.menuItem.name} className="w-14 h-14 object-cover rounded-xl flex-shrink-0 border border-brand-gold/10" />}
                        <div className="flex-1 min-w-0">
                          <h4 className="font-heading text-sm sm:text-base font-bold text-white truncate pr-6">{item.menuItem.name}</h4>
                          <span className="text-[10px] uppercase font-sans tracking-widest text-brand-gold/75 block mt-0.5">{item.menuItem.category}</span>
                          <span className="font-body text-xs text-brand-gold/90 block mt-1">{priceCombined}</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <div className="flex items-center bg-brand-dark border border-brand-gold/15 rounded-full text-xs overflow-hidden py-0.5">
                            <button onClick={() => onUpdateQuantity(item.menuItem.id, -1)} className="px-2 py-1 text-brand-cream/80 hover:bg-white/5 font-bold cursor-pointer">
                              <Minus className="w-3 h-3" />
                            </button>
                            <span className="px-2.5 font-bold font-body text-white text-xs">{item.quantity}</span>
                            <button onClick={() => onUpdateQuantity(item.menuItem.id, 1)} className="px-2 py-1 text-brand-gold hover:bg-white/5 font-bold cursor-pointer">
            ...
            <div className="p-5 border-t border-brand-gold/15 bg-brand-dark">
              <div className="space-y-3 mb-5">
                <div className="flex justify-between items-center text-brand-cream/70 text-sm">
                  ...
                  <span className="font-body">{totalCount} items</span>
                </div>
                <div className="flex justify-between items-end border-t border-brand-cream/10 pt-3">
                  <div className="text-base font-heading font-bold text-white uppercase tracking-wider">Total Estimated</div>
                  <div className="text-right">
                    <div className="text-2xl font-body font-bold text-brand-gold leading-none">₦{totalPriced.toLocaleString()}</div>
                    {hasPriceOnRequest && <span className="text-[10px] text-brand-red font-sans uppercase font-bold block mt-1 tracking-widest">+ Price on request</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="text-center font-sans text-[10px] text-brand-cream/40 mt-3 uppercase tracking-wider">Opens WhatsApp redirecting to DeltaPot lines</div>
            </div>
          )}
        </motion.div>
      </div>
