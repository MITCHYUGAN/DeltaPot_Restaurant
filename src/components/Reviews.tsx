      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {GOOGLE_REVIEWS.map((review) => (
            ...
            >
              <div className="space-y-4">
                  ...
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      ...
                      className={`w-3.5 h-3.5 ${
                        i < review.rating ? 'fill-current' : 'text-brand-cream/15'
                      }`}
                    />
                  ))}
                </div>
                ...
                <p className="text-xs sm:text-sm text-brand-cream/80 leading-relaxed font-sans font-light italic">
                  "{review.text}"
                </p>
              </div>
              ...
              <div className="flex items-center gap-3.5 pt-5 border-t border-brand-gold/10 mt-6 md:mt-8">
                {review.avatar && (
                  <img
                    src={review.avatar}
                    alt={review.name}
                    className="w-10 h-10 rounded-full object-cover border border-brand-gold/20 flex-shrink-0"
                  />
                )}
                <div className="min-w-0">
                  <h4 className="font-serif text-sm font-bold text-white group-hover:text-brand-gold transition-colors duration-200">
                    {review.name}
                  </h4>
                  <p className="text-[10px] text-brand-cream/50 uppercase font-sans tracking-wider mt-0.5 font-semibold">
                    {review.role}
                  </p>
                  <p className="text-[9px] text-brand-cream/35 font-sans mt-0.5">
                    {review.date}
                  </p>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-5 bg-gradient-to-b from-brand-dark to-brand-brown/60 border border-brand-gold/10 rounded-3xl p-6 mt-12 max-w-xl mx-auto text-center sm:text-left">
          <div className="px-6 py-4 bg-brand-charcoal border border-brand-gold/15 rounded-2xl text-center flex-shrink-0 shadow-md">
            <span className="text-3xl font-bold text-white font-serif block">4.1</span>
          ...
          <div>
            <h4 className="font-serif text-base font-bold text-white">Rated 4.1 Stars on Google Maps</h4>
